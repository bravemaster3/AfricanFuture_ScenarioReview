...
authors <- read_xlsx("data/AFILKScenarios_Literature_Database.xlsx", sheet="countries_only")
...
country_pairs[country_pairs$country1 %in% c("Cameroon", "Côte d’Ivoire", "Ethiopia", "Ghana", 
                                            "Kenya", "Senegal", "South Africa", "Tanzania"),"origin"] <- "From Africa"

'%ni%' <- Negate('%in%')
country_pairs[country_pairs$country1 %ni% c("Cameroon", "Côte d’Ivoire", "Ethiopia", "Ghana", 
                                            "Kenya", "Senegal", "South Africa", "Tanzania"),"origin"] <- "From Oversees"
...
write.table(country_pairs,"data/country_pairs.csv",
            sep=",", row.names = F)
######################################################
###################PLOTING#########################
###################################################
...
p_edges_dest_afr <- ggplot(country_pairs[country_pairs$origin =="From Oversees",]) +
  geom_curve(aes(x = x, y = y, xend = xend, yend = yend,     # draw edges as arcs
                 size = weight),arrow=arrow(length = unit(0.03, "npc"), type = "closed"),
...
# Export
png("plots/network_map_noLegend_2_colors.jpg",width = 20, height = 10, units = "cm", pointsize = 3, res=500) 
print(p)
dev.off()
